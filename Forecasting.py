...
st.set_page_config(page_title='Weather Forecasting', page_icon="🌥️")
...
if(st.button("SUBMIT")):
    try:
        ...
        for item in y["daily"]:
            ...
            sunset.append((datetime.datetime.utcfromtimestamp(item["sunset"])+timedelta(hours=5, minutes=30)).strftime('%H:%M'))
        ...
        def bargraph():
            ...
            fig.update_layout(xaxis_title="Dates",yaxis_title="Temperature",barmode='group',margin=dict(l=70, r=10, t=80, b=80) )
            st.plotly_chart(fig)
        ...
        def linegraph():
            ...
            fig.update_layout(xaxis_title="Dates",yaxis_title="Temperature")
            st.plotly_chart(fig)
        ...
        table2=go.Figure(data=[go.Table(columnwidth=[1,2,1,1,1,1],header=dict(values=['<b>DATES</b>','<b>WEATHER CONDITION</b>','<b>WIND SPEED</b>','<b>PRESSURE<br>(in hPa)</b>','<b>SUNRISE<br>(in IST)</b>','<b>SUNSET<br>(in IST)</b>']
